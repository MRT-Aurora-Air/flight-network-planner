...
pub fn run(config: &mut Config, fd: &FlightData) -> Result<Vec<(Flight, i8)>> {
    ...
    let mut flights: Vec<(Flight, i8)> = vec![];
    while let Some((g1, g2, s, ty)) = possible_flights.pop() {
        ...
        })
        {
            trace!(
                ...
                g1.size, g1.airport, g1.code, g2.airport, g2.code
            );
            continue;
        }
        ...
        {
            debug!(
                "Rejected ({}): {} {} <-> {} {} ({1} hit max limit of {})",
                g2.size, g1.airport, g1.code, g2.airport, g2.code, g1_hardmax
            );
            continue;
        }
            ...
            >= g2_hardmax
        {
            debug!(
                "Rejected ({}): {} {} <-> {} {} ({1} hit max limit of {})",
                g1.size, g2.airport, g2.code, g1.airport, g1.code, g2_hardmax
            );
            continue;
        }
        if destinations
            ...
            >= max1 as usize {
            debug!(
                "Rejected ({}): {} {} <-> {} {} ({1} hit max type limit of {})",
                g2.size, g1.airport, g1.code, g2.airport, g2.code, max1
            );
            continue;
        }
        if destinations
            ...
            debug!(
                "Rejected ({}): {} {} <-> {} {} ({1} hit max type limit of {})",
                g1.size, g2.airport, g2.code, g1.airport, g1.code, max2
            );
            continue;
        }

        destinations
            .entry(g1.airport.to_owned())
            .or_insert(vec![])
            .push(g2.airport.to_owned());
        destinations
            .entry(g2.airport.to_owned())
            .or_insert(vec![])
        ...
        info!(
            "{} ({}): {} -> {}, {}",
            flight1.flight_number, g1.size, g1.airport, g2.airport, s
        );
        flights.push((flight1, s));
        ...
        info!(
            "{} ({}): {} -> {}, {}",
            flight2.flight_number, g2.size, g2.airport, g1.airport, s
        );
        flights.push((flight2, s));
        possible_flights = sort_gates(possible_flights);
    }
