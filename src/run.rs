        ...
        .filter_map(|(g1, g2, _, ty)| {
            ...
            if destinations
                ...
                || destinations
                    ...
                    .unwrap_or(&vec![])
                    .iter()
                    .filter(|d| (&g1.airport, *d).get_flight_type(&mut config, &fd).unwrap() == ty)
                    .count()
                    >= max as usize
                || destinations
                    .get(&*g2.airport.to_owned())
                    .unwrap_or(&vec![])
                    .iter()
                    .filter(|d| (&g2.airport, *d).get_flight_type(&mut config, &fd).unwrap() == ty)
                    .count()
                    >= max as usize
            {
                return None;
            ...
            let flight = Flight {
                flight_number: if let Some(fn_) = match ty {
                    ...
                    FlightType::ExistingH2N | FlightType::NonExistingH2N => {
                        h2n_fng.entry(g1.airport.to_owned()).or_insert_with(|| {
            ...
            info!(
                "{} ({}): {} -> {}",
                flight.flight_number, g1.size, g1.airport, g2.airport
            );
            Some(flight)
        })
        .collect::<Vec<_>>())
