    ...
    pub fn from_gatelogue() -> Result<Self> {
                ...
                $data
                    ...
                    .map(|a| {
                        Ok((
                            a,
                            a.get("type")
                                .ok_or(anyhow!("No `type`"))?
                                .as_str()
                                .ok_or(anyhow!("`type` not string"))?,
                        ))
                    })
        ...
        let flights = filter_from_data!(data, "AirFlight")
            .map(|a| {
                let airline_id = a
                    .get("airline")
                    .ok_or(anyhow!("No `airline`"))?
                    .as_number()
                    .ok_or(anyhow!("`airline` not number"))?;
                let airline_name = data
                    .get(&airline_id.to_string())
                    .ok_or(anyhow!("airline {airline_id} does not exist"))?
                    .get("name")
                    .ok_or(anyhow!("No `name`"))?
                    .as_str()
                    .ok_or(anyhow!("`name` not string"))?;

                let flight_number = a
                    .get("codes")
                    .ok_or(anyhow!("No `codes`"))?
                    .get(0)
                    .ok_or(anyhow!("No codes"))?
                    .as_str()
                    .ok_or(anyhow!("`codes` elements not string"))?;

                let airport_codes = a
                    .get("gates")
                    .ok_or(anyhow!("No `gates`"))?
                    .as_array()
                    .ok_or(anyhow!("`gates` not array"))?
                    .iter()
                    .map(|a| {
                        let gate_id = a
                            .as_number()
                            .ok_or(anyhow!("`gates` elements not number"))?;
                        let airport_id = data
                            .get(&gate_id.to_string())
                            .ok_or(anyhow!("gate {gate_id} does not exist"))?
                            .get("airport")
                            .ok_or(anyhow!("No `airport`"))?
                            .as_number()
                            .ok_or(anyhow!("`airport` not number"))?;
                        let airport_code = data
                            .get(&airport_id.to_string())
                            .ok_or(anyhow!("airport {airport_id} does not exist"))?
                            .get("code")
                            .ok_or(anyhow!("No `code`"))?
                            .as_str()
                            .ok_or(anyhow!("`code` not string"))?;
