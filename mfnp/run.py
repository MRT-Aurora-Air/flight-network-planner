...
def flight_num_generator(config: utils.Config, flight_nums: list[utils.FlightNumber], mode: str,
                         code_: str | None = None) -> Generator[utils.FlightNumber, None, None]:
    ...
    utils._warn(f"Not enough flight numbers for {mode} flights" + (f" from {code_}" if code_ else ""))


def get_connection_type(config: utils.Config, origin: str, dests: list[str]) -> dict[str, int]:
    out = {
        "h2h": 0,
        "h2n": 0,
        "n2n": 0
...
def run(config: utils.Config, output_format: str= "json", nocache: bool=False):
    ...
    for airline in config.ignored_airlines:
        ...
        else: utils._warn(f"Airline `{airline}` doesn't exist, maybe you mean: {utils._gcm(airline, data.keys())}")

    # double-check all airport codes
    utils._debug("Double-checking airport codes")
    for code in config.gates.keys():
        if code not in airports:
            utils._warn(f"Airport `{code}` doesn't exist, maybe you mean: {utils._gcm(code, airports)}")
    for code in config.hubs:
        if code not in config.gates.keys():
            utils._warn(f"Airport `{code}` has no gates but is stated as a hub, maybe you mean: {utils._gcm(code, config.gates.keys())}")
    ...
    if output_format == 'json':
        ...
        for flight in flight_plan:
            if flight.code in processed_flights: continue
            out.append({
                "number": flight.code,
                "size": flight.size,
                "code1": flight.orig,
